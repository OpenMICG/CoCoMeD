...
class VQABase(Dataset):
    """Base class for VQA datasets基本的VQA数据集读取"""
    def __init__(self, subset, config, dataset_visual):
        ...
        self.path_processed = jp(config['path_qa'], 'processed')#载入字典路径
        #os.path.exists用来判断某个路径是否存在
        if not os.path.exists(self.path_processed) or len(os.listdir(self.path_processed))<1 or (subset == 'train' and config['process_qa_again']):
            self.pre_process_qa() # pre-process qa, produce pickle files字典生成

        # load pre-processed qa 字典载入
...
class VQARegionsSingle(VQABase):
    ...
    def transform(self, image, mask, size):
        ...
        if self.subset == 'train': # for training images, do random crops and horizontal flips. For other sets, only resize
            ...
            if random.random() > 0.5:
                ...
                mask = TF.hflip(mask)#水平翻转

            if random.random() > 0.5:
                angle = random.randint(-10, 10) # define angle
                image = TF.rotate(image, angle)#旋转角度
                mask = TF.rotate(mask, angle)#旋转角度
    ...
    def __getitem__(self, index):
        ...
        # get question
        sample['question_id'] = item_qa['question_id']
        sample['question'] = torch.LongTensor(item_qa['question_word_indexes'])#longtensor是64位整型变量
    ...
    # define preprocessing method for qa pairs qa对的预先处理生成
    def pre_process_qa(self):
...
class VQARegionsSingleMainSub(VQARegionsSingle):
    ...
    def __init__(self, subset, config, dataset_visual, dataset_mask):
        ...
        #得到了所有的qa对，即每一个qa对中存在两个问题，要么是main-sub，要么是ind-ind
...
class SlakeVQAMainSub(Dataset):
    """Class for Slake dataset getting samples that contain a main question and a subquestion. """
    def __init__(self, subset, config, dataset_visual):
        self.subset = subset
        self.config = config
        self.dataset_visual = dataset_visual
        self.path_annotations_and_questions = config['path_qa']#qa路径
        self.path_processed = jp(config['path_qa'], 'processed')#字典路径
        if 'augment' not in config:
            config['augment'] = False
        self.augment = config['augment']#数据增强
        if not os.path.exists(self.path_processed) or len(os.listdir(self.path_processed)) < 1 or (\
                subset == 'train' and config['process_qa_again']):
            self.pre_process_qa()
        ...
        print('Building pairs')
        for mq in tqdm(main_questions): # iterate through all main questions
            main_image = mq['img_name']
            main_role_num = mq["role_num"]
            # get all sub-questions for current main question
            sub_questions = [e for e in self.dataset_qa if e["img_name"] == main_image and e["role"] == 'sub' and e['role_num'] == main_role_num]
            for sq in sub_questions:
                self.dataset_qa_pairs.append((mq, sq))
    ...
    #字典生成
    def pre_process_qa(self):
        data_train = json.load(open(jp(self.path_annotations_and_questions, 'trainEnVQA_new.json'), 'r'))
        data_val = json.load(open(jp(self.path_annotations_and_questions, 'validateEnVQA_new.json'), 'r'))
        data_test = json.load(open(jp(self.path_annotations_and_questions, 'testEnVQA_new.json'), 'r'))

        sets, maps = vqa2_helper.process_qa_SLAKE(self.config, data_train, data_val, data_test)
    ...
    def __getitem__(self, index):
        ...
        for item in self.dataset_qa_pairs[index]:
            sample = {}
            # get visual
            visual = self.dataset_visual.get_by_name(item['img_name'])['visual']

        if self.augment:
            sample['visual'] = self.transform(visual, 448)
        else:
            sample['visual'] = visual
