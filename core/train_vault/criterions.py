...
def fcn2(non_avg_ce, flag, device, gamma=0.5, exp=True):
    ...
    #non_avg_ce表示交叉熵损失，
    #flag表示是否为main-sub（1）或ind（0）
    if torch.count_nonzero(flag) == 0: # if all flags are 0 (all questions are ind), return 0, otherwise NAN is generated.
        return torch.tensor(0)

    # separate into even and odd indexes (main vs sub or ind vs ind)分为奇数和偶数的下标
    #len(tensor)是判断tensor有几个维度
    ...
    if exp:
        ...
        func = sub_ce*(gamma - main_ce)#+main_ce*(gamma - sub_ce)
