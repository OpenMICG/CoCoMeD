...
def get_top_answers(answers, nans=2000):
    counts = Counter(answers).most_common()#将答案的出现频率由高到低去排列
    top_answers = [elem[0] for elem in counts[:nans]]#只取出前nans个答案，而不要出现的次数
    return top_answers#将答案按照出现的次数从上往下排列并取出
...
def clean_text(text):
    ...
    t = str.maketrans(dict.fromkeys(punc_list, " "))#dict.fromkeys将生成以punc_list为键空格为值的键值对，str.maketrans将其变为unicode编码。
    text = text.translate(t)#将上述的字符和数字替换成空格

    # replace single quote with empty character
    t = str.maketrans(dict.fromkeys("'`", ""))
    text = text.translate(t)#将引号'`替换成空格
...
def add_UNK_token_and_build_word_maps(data, min_word_frequency):
    ...
    all_words_in_all_questions = list(itertools.chain.from_iterable(elem['question_tokens'] for elem in data))#itertools.chain.from_iterable用来组合多个迭代器
    # count and sort 
...
#为SLAKE添加UNK和创建单词谱
def add_UNK_token_and_build_word_maps_SLAKE(data, min_word_frequency):
    # function to build vocabulary from question words and then build maps to indexes
    data_all = data[0]+data[1]+data[2]
    all_words_in_all_questions= list(itertools.chain.from_iterable(elem['question_tokens'] for elem in data_all))#itertools.chain.from_iterable用来组合多个迭代器
    ...
    vocab_words_in_questions = [elem[0] for elem in counts if elem[1] > min_word_frequency]
    # add_entry for tokens with UNK to data
    for elem in tqdm(data[0]):
        elem['question_tokens_with_UNK'] = [w if w in vocab_words_in_questions else 'UNK' for w in elem['question_tokens']]
    # build maps
    vocab_words_in_questions.append('UNK') # Add UNK to the vocabulary
    map_word_index = {elem:i+1 for i,elem in enumerate(vocab_words_in_questions)} #* added +1 in i34 to avoid same symbol of padding
    map_index_word = {v:k for k,v in map_word_index.items()} # * changed to dict in i34 because list does not work anymore due to first index being 1 instead of 0
    return data[0], map_word_index, map_index_word
