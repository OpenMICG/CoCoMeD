...
def collater(batch):
    ...
    if torch.is_tensor(batch[0]):
        ...
        res1 = batch[0][0]
        res = dict.fromkeys(res1.keys())
        for k in res.keys():
            res[k] = [s[0][k] for s in batch]
...
def collater_mainsub(batch):
    ...
    if isinstance(batch[0], tuple): # each element of the batch should be a tuple with a main and a sub-question or two independent questions判断是不是tuple类型
        for elem in batch:
            for sample in elem:
...
def get_classif_loader(subset, config, shuffle=False):
    ...
    dataloader = DataLoader(    dataset_classif,
                                ...
                                shuffle=shuffle,
                                num_workers=config['num_workers'],#线程数
                                pin_memory=config['pin_memory'],#当内存充足情况下设置为true，内存不足时采用交换内存时使用false
                                collate_fn=collater
                            )
    return dataloader
...
def get_vqa_loader(subset, config, shuffle=False):
    ...
    #subset表示哪个子集
    # create visual dataset for images可以通过get_by_name方法获取图像或者直接通过下标获得图像
    dataset_visual = visual_SLAKE.get_visual_dataset(subset, config)#dataset中包含图像名称、图像路径、图像的具体像素值
    ...
    ###############################################################################
        ...
        pass
    elif config['dataset'] == 'SLAKE':
        dataset_vqa = vqa.SlakeVQAMainSub(subset, config, dataset_visual)
    else:
        raise ValueError("Unknown dataset")

    if config['mainsub'] and subset != 'test':
        collater_fn = collater_mainsub#做每个batch的数据整合
    else:
