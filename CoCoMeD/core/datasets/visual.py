...
class ImagesDataset(Dataset):
    ...
    def __init__(self, subset, config, transform=None):
        self.subset = subset
        self.path_img = jp(config['path_img'], subset)#将路径组合（jp为os.path.join）
        self.transform = transform
        self.images = os.listdir(self.path_img) # list all images in folder列出文件夹中所有的图片名称
...
def default_transform(size):
    ...
    transform = transforms.Compose([
        ...
        transforms.CenterCrop(224),#在图片的中心位置进行裁剪
